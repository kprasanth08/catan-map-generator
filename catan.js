...
HexTile.prototype.drawNumber = function() {
	...
	drawingContext.beginPath();
	drawingContext.arc(this.xCenter - (0.3*size), this.yCenter +(0.5*size), 0.3 * size,
		0, 2 * Math.PI, false);
	drawingContext.closePath();
	...
	var fontSizePt = Math.ceil(30/40*(.4*size-8)+5);
	...
	drawingContext.fillText(
		...
		this.xCenter - (0.3*size), this.yCenter +(0.5*size) + Math.ceil( 0.85 * fontSizePt/2 )
	);
